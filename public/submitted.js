...
if (boroughCookie != null) { // checks if there's a cookie with info on this EXACT borough
    ...
    (async () => {
        ...
        if (pricesCache != null) { // checks if there's a cookie with prices from the homepage (info on all boroughs)
            ...
            .then(resp => resp.json())
            .then(json => {
                let prices = [json.items[0]['ukhpi:averagePrice'][0], json.items[1]['ukhpi:averagePrice'][0]],
                higherPrice = 1
